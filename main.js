...
function UpdateRunTime(date) {
	...
	connection.query(`SELECT TABLE_NAME FROM information_schema.tables WHERE TABLE_SCHEMA = ?;`, [appConfig[2][1]], function (err, rows, fields) {
		...
		mntApps.forEach(mntApp => {
			...
			console.log(`SELECT * FROM ${mntApp} WHERE StartTime${toQueryString(date)};`);
			connection.query(`SELECT * FROM ${mntApp} WHERE StartTime${toQueryString(date)};`, function (error, res) {
				if (error !== null) {
...
function toQueryString(date) {
	let former = date;
	let latter = new Date();
	latter.setDate(former.getDate() + 1);
