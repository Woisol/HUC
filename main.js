...
// ipcMain.on("UpdateRunTime", UpdateRunTime)
function UpdateRunTime() {
	...
	connection.query(`SELECT TABLE_NAME FROM information_schema.tables WHERE TABLE_SCHEMA = ?;`, [appConfig[2][1]], function (err, rows, fields) {
		...
		mntApps.forEach(mntApp => {
			...
			connection.query(`SELECT * FROM ${mntApp} ORDER BY StartTime DESC;`, function (error, resultes) {
				if (error) { console.log("Failed to Read DB"); return; }
