...
function TimeCard({ data, pxPerMin }) {
	...
	return (
		<div className="absolute flex flex-col justify-center w-full text-xs text-center text-transparent transition-all bg-gray-400 hover:text-black" style={{ height: `${(data[1] - data[0]) * pxPerMin}px`, bottom: `${data[0] * pxPerMin}px` }} >
...
export default function AppRunTimeShowcase({ data }) {
	...
	return (
		<div className='flex flex-col items-center w-16 h-full p-2 mx-2 transition-all shadow-xl sm:w-20 md:w-28 rounded-2xl hover:shadow-2xl' style={{ backgroundColor: `${data[2]}` }}>
			<div className="relative w-3 h-full transition-all bg-gray-300 rounded-lg shadow-lg AppRunTimeShowcase sm:w-5 md:w-7 right-2 hover:shadow-2xl">
			...
			<div className="relative w-10 h-10 p-0 mx-1 transition-all bg-blue-300 sm:p-2 sm:w-16 sm:h-16 md:w-20 md:h-20 rounded-2xl hover:shadow-2xl hover:bg-blue-400 group" >
				...
				<span className='absolute bottom-0 hidden p-1 text-xs text-center -translate-x-1/2 bg-white rounded-md opacity-75 left-1/2 w-fit group-hover:block'>{data === undefined ? null : data[0]}</span>
			</div>
		</div>
	)
}
