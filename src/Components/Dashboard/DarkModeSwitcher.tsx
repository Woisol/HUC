...
export default function DarkModeSwitcher({ isDarkMode, setIsDarkMode }) {
	...
	return (
		...
		<Switch
			...
			{isDarkMode ? (
				<span className="absolute overflow-hidden duration-500 bg-gray-400 rounded-full translate-x-11 w-9 h-9" style={{ transitionProperty: 'color, transform' }}>
					<span className={'absolute -translate-x-1 -translate-y-[2px] w-[30px] h-[30px] transition-all duration-700 bg-gray-700 rounded-full'}></span>
				</span>

			) : (
				<span className="absolute bg-yellow-200 border-[4px] border-yellow-100 duration-500 translate-x-0 rounded-full w-9 h-9 " style={{ transitionProperty: 'color, transform' }}>
