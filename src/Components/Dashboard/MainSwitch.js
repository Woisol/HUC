...
export default function MainSwitch(props) {// isMonitorRunning, setState
	...
	return (
		<div className="h-48 w-fit absolute -top-32 left-10 flex items-center " style={{ transition: "0.8s" }}>
			<img onClick={RequestMonitorStateChange} className={props.isMonitorRunning ? "h-48 w-48 p-3 shadow-2xl bg-green-300 rounded-2xl " : "h-48 w-48 p-3 shadow-2xl bg-red-300 rounded-2xl"} style={{ transition: "0.8s" }} src={Logo} alt={props.isMonitorRunning ? "Running" : "Stopped"} />
